...
<div class="kit-doc">
    <!--这里写页面的代码-->
    <blockquote class="layui-elem-quote">这是关于公告管理页面</blockquote>
    <div class="kit-doc-title">
        <fieldset>
            <legend><a name="blockquote">公告管理</a></legend>
        </fieldset>
    </div>
...
<script>
    ...
    layui.use('table', function() {
        ...
        table.render({
            elem: '#test'
            , url: '<%=basePath%>notice/noticeGetAllJson'
            , method: 'post'
            , cols: [[
                {type: 'numbers', title: '序号', fixed:'left'},
                {field: 'id', align: 'center', width:60, title: '编号'}
                , {field: 'title', title: '标题', width:500}
                , {field: 'time', title: '发布时间', width: 120, sort: true}
                , {field: 'url', title: '公告地址', width: 280}
        ...
        table.on('tool(demo)', function (obj) {
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    type: 2,
                    content: '<%=basePath%>notice/goUpdateNotice?id=' + data.id,
                    area: ['600px', '450px'],
                    maxmin: true,
                    end: function () {
                        location.reload();
                ...
                layer.confirm('真的要删除么？', function (index) {
                    // 写删除方法
                    $.post("<%=basePath%>notice/delNotice", {"id": data.id}, function (data) {
                        if (data.code == "200") {
