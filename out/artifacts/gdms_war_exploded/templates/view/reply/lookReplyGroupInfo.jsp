...
<div class="kit-doc">
    <!--这里写页面的代码-->
    <blockquote class="layui-elem-quote">这是答辩组的信息</blockquote>
...
<script>
    ...
    layui.use('table', function() {
        ...
        table.render({
            elem: '#test'
            , url: '<%=basePath%>reply/replyGroupGetAllJson'
            , method: 'post'
            , cols: [[
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'teacher'){
                var index = layer.open({
                    type: 2,
                    content: '<%=basePath%>reply/goDReplyGroupTeacher?id='+data.id,
                    area: ['800px', '450px'],
                    maxmin: true,
                    end: function () {
                        location.reload();
                ...
                var index = layer.open({
                    type: 2,
                    content: '<%=basePath%>reply/goDReplyGroupStudent?id='+data.id,
                    area: ['800px', '450px'],
                    maxmin: true,
                    end: function () {
                        location.reload();
